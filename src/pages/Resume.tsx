...
import { Download, FileText, User, GraduationCap, Briefcase, Award, BookOpen, Trophy, Code, ArrowLeft } from 'lucide-react';
...
const Resume = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="py-20 bg-card/20 relative" ref={ref}>
            ...
            >
              ...
              >
                ...
                {resumeData.experience.map((exp, index) => (
                  <div key={index} className="mb-6 p-4 bg-background/50 rounded-lg">
                    ...
                    {exp.achievements && (
                      <div className="mt-3">
                        <p className="text-foreground text-sm font-medium mb-2">Key Achievements:</p>
                        {exp.achievements.map((achievement, achIndex) => (
                          <p key={achIndex} className="text-muted-foreground text-xs mb-1">
                            • {achievement}
                          </p>
                        ))}
                      </div>
              ...
              >
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-xl font-bold text-foreground">Academic & Personal Projects</h3>
                </div>
                {resumeData.projects.map((project, index) => (
                  <div key={index} className="mb-4 p-4 bg-background/50 rounded-lg">
                    <h4 className="font-semibold text-foreground">{project.title}</h4>
                    <p className="text-accent text-sm font-medium">{project.technologies}</p>
                  ...
                  {Object.entries(resumeData.technicalSkills).map(([category, skills]) => (
                    <div key={category} className="p-3 bg-background/50 rounded-lg">
                      <h4 className="font-medium text-foreground text-sm mb-2">
                        {category.replace(/([A-Z])/g, ' $1').trim().charAt(0).toUpperCase() + 
                         category.replace(/([A-Z])/g, ' $1').trim().slice(1)}
                      </h4>
